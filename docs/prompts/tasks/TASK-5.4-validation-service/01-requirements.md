...
- Treat `.pending` as an unfilled required outcome: required steps must reject `.pending` so judges cannot skip required test steps. `.pass`, `.fail`, and `.notApplicable` satisfy the outcome gate, with `.fail` then triggering the separate notes rule.
- Validation issues should expose deterministic issue codes plus localization keys and interpolation arguments. Fallback English messages may remain for current callers/tests, but raw UI strings should not be the long-term service contract.
- Evidence validation should be policy-shaped. The current boolean `requiresEvidence` can map to a minimum of one metadata attachment for this slice, but the design must leave room for multiple required metadata records and counts by `EvidenceMediaType`.
