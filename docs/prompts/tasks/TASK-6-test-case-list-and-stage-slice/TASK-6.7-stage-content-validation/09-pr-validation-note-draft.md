...
- Current branch during preparation was `develop`; expected task branch is `codex/task-6.7-stage-content-validation`.
- Static source review found:
  - Garage: 1 section, 9 cases, 9 steps, 1 measurement step.
  - EV: 8 sections, 119 cases, 165 steps, 16 measurement steps, 17 `CAUTION: ENERGIZED` cases.
  - Egress: 1 section, 1 case, 21 steps, 4 measurement steps.
  - Rain: 1 section, 1 energized case, 22 steps, 3 measurement steps.
...
- `[static]` Inspected `Design/Resources/InspectionEvent/01_garage_inspection.json`, `04_ev_inspection.json`, `05_egress_test.json`, and `06_rain_test.json` with `jq`.
