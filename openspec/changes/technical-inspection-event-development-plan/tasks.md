...
- [ ] 2.3 Add `.agents/planner.md` with incremental feature slicing, manual validation checkpoints, task branch naming, squash merge, and documented git workflow responsibilities.
...
- [ ] 6.1 Add failing unit tests for `InspectionStage`, `InspectionSection`, bundled JSON decoding, display ordering, stable IDs, unsupported step type errors, and malformed JSON failures.
...
- [ ] 9.4 Ensure each implementation PR includes manual validation notes, test command output, linked `.feature` scenarios, and squash-merge guidance.
- [ ] 9.5 Keep task branches focused on one feature slice and avoid unrelated refactors.
