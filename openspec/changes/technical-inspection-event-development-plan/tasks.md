...
- [ ] 10.4 Add a local team catalog and no-default-mock production launch. A user can create a team entry with display name and car number, and `MockInspectionData.teams` remains available only for tests, previews, or explicit debug fixtures.
- [ ] 10.5 Add local session lifecycle for one team: start session, record `startedAt`, end/complete session, record `endedAt`, persist active session state, and restore the active session after relaunch.
